...
void Input::setElement(Element *element) { this->element = element; }

void Input::setIsFocusing(bool focus) {
    isFocusing = focus;
...
void Input::setAdjustPosition(bool adjust) { adjustPosition = adjust; }

void Input::handleKeyboardHeightChange(double height) {
    keyboardHeight = height;
    handleElementPositionWhenFocus();
}

void Input::handleElementPositionWhenFocus() {
    if (isFocusing && adjustPosition) {
        auto scrollElement = element->closestVerticalScrollElement();
        auto windowHeight = element->document()->GetBodyElement()->OffsetHeight();
...
void Input::setBodyTranslateY(double y) {
    const auto body = element->document()->GetBodyElement();
